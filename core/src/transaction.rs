...
impl CellKey {
    ...
    pub fn recover(&self) -> (H256, u32) {
        Self::deconstruct(&self.0)
    }

    pub fn deconstruct(bytes: &[u8]) -> (H256, u32) {
        let hash = H256::from_slice(&bytes[..32]).expect("unreachable");
        let mut le_bytes = [0u8; 4];
        le_bytes.copy_from_slice(&bytes[32..36]);
