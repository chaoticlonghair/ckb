...
use crate::rocksdb::RocksDBMemoryStatistics;
...
lazy_static::lazy_static! {
    ...
    pub(crate) static ref CONTROL_HANDLE: Arc<RwLock<Option<Sender<(Arc<String>, MeasureRecord)>>>> = Arc::new(RwLock::new(None));
    ...
    for (tag, measure) in stats {
        match measure {
            ...
            MeasureRecord::SpooledHashMapWithRocksDB {
                ...
                "{:20} {{ tag: {:40}, len: {:8}, cap: {:8}, disk: {:8}, db: {} }}",
                "SpooledHashMap",
                tag,
                mem_len,
                mem_cap,
